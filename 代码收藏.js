...
/* 自定义滚动条 */
drag.onmousedown=function (ev){ //按住滚动条
    ...
    document.onmousemove=function (ev){ //按住并移动
        var e=ev||window.event;
...
if (str.indexOf('firefox')!=-1) {//火狐浏览器
    out.addEventListener('DOMMouseScroll',function (e){ //out是con、box的父盒子。
    ...
    out.onmousewheel=function (ev){
        ...
        if (e.wheelDelta>0) { //正值上滚
            var t=con.offsetTop+20;
            if (t>=0) {
                t=0;
            };
            if (t<=-(con.clientHeight-out.clientHeight)) { //总滚动高度 = 内容盒子con高度 - 父盒子out的有限高度
                t=-(con.clientHeight-out.clientHeight);
            };
            var scale=t/(con.clientHeight-out.clientHeight);
            var top=scale*(box.clientHeight-drag.clientHeight);
